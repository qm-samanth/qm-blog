  ...
  return (
    <article className="rounded-sm overflow-hidden hover:shadow-lg transition-shadow duration-300 flex flex-col h-full bg-white">
        ...
        <Link href={`/posts/${post.slug}`}>
          <div className="relative h-80 bg-gray-200 overflow-hidden group">
            ...
            {post.status !== "PUBLISHED" && (
              <div className="absolute top-3 right-3 bg-white/90 text-gray-900 px-3 py-1 rounded-sm text-xs font-semibold">
                {post.status}
              </div>
            )}
          </div>
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        {/* Preview Text */}
        {preview && (
          <p className="text-sm text-gray-600 mb-4 line-clamp-7 flex-grow">
            {preview}
          </p>
        )}
